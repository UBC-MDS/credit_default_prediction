...
Our model's test $f1$ score is `r py$final_tuned_classification_f1_score` with recall and precision rate being `r py$final_tuned_classification_recall`, `r py$final_tuned_classification_precision` respectively. We also report average_precision_score (AP) as it summarizes a precision-recall curve: the weighted mean of precisions achieved at each threshold, with the increase in recall from the previous threshold used as the weight. This score tells us the overall model performance on imbalance dataset. Overall, we conclude that the model can correctly classify default payments roughly half of the time. 
...
### Understanding the relation between features   
